...
# 2. Find Section 3.2 and 3.3
print("Locating Section 3.2 and 3.3...")
idx_3_2 = -1
idx_3_3 = -1
...
print(f"Index of 3.2 App for Robot: {idx_3_2}")
print(f"Index of 3.3 Android Customer App: {idx_3_3}")

if idx_3_2 == -1 or idx_3_3 == -1:
    print("Error: Could not locate Section 3.2 or 3.3")
    sys.exit(1)

# Remove all existing paragraphs between 3.2 and 3.3
...
print(f"Cleared {len(paragraphs_to_remove)} existing paragraphs in Section 3.2.")

# Helper functions to insert content before anchor (p_3_3)
def add_h4(title):
...
print("Writing English specifications with Vietnamese UI buttons for all 8 sub-modules...")

# ==============================================================================
# 3.2.1 System Startup & Idle Kiosk Mode
# ==============================================================================
add_h4("3.2.1 System Startup & Idle Kiosk Mode")
add_p("[TODO: ADD SCREENSHOT - System Startup Animation Screen (Hardware diagnostics & sensor check 3s)]", red_marker=True)
add_images([img("01_Welcome_Kiosk_Idle.jpg")], width_inches=2.6)
add_p("[TODO: ADD SCREENSHOT - Standby Advertising Mode Screen (Full-screen standby video/banner loop during robot idle)]", red_marker=True)

add_h4("Function Trigger")
add_p("System initiates automatically when the robot is powered on or the Android Robot application (com.truongduyyyy.mobilerobot) launches. Following a 3-second diagnostic sequence, the system enters the Welcome Screen. Standby Advertising Mode triggers automatically after 60 seconds of user inactivity.", bold_prefix="System Startup & Standby Trigger:")
add_p("Tap anywhere on the Welcome Screen or click the \"Khuyến mãi hôm nay\" (Today's Promotions) button at the bottom to initiate the shopping interaction.", bold_prefix="User Tap Trigger:")
...
add_p("Supermarket shoppers, Technical staff / Robot operators.", bold_prefix="Actors / Roles:")
add_p("Android Robot Kiosk Application (React Native / Expo SDK 57), Text-to-Speech Engine (RobotVoiceService), Customer Session Manager (CustomerSessionContext).", bold_prefix="System:")
...
add_p("Real-time digital clock (22:24) and date (Thứ Năm, 17/09/2026); animated 2D robot avatar with radar wave pulse effect; title badge \"TRỢ LÝ SIÊU THỊ THÔNG MINH\"; prominent call-to-action button \"[ CHẠM ĐỂ BẮT ĐẦU ]\"; promotional shortcut button \"Khuyến mãi hôm nay\" with gift box icon.", bold_prefix="Interface (Based on UI Mockups):")

add_h4("Normal Flow")
add_p("1. The application starts up, displaying the robot logo with an initialization spinner and status text: '⚡ Đang khởi động hệ thống robot...', '🔍 Đang kiểm tra Camera AI & Cảm biến...', '🎙️ Đang kích hoạt cổng nhận diện giọng nói...'.")
add_p("2. After 3 seconds of diagnostics, the Welcome Screen fades in smoothly. Robot Assistant broadcasts a greeting via speaker: \"Chào mừng quý khách đến với Smart Market Bot! Tôi có thể giúp gì cho bạn?\" (Welcome to Smart Market Bot! How may I assist you?).")
...
add_images([img("03_Role_Selection.jpg")], width_inches=2.5)
add_p("[TODO: ADD SCREENSHOT - Active Face ID Scanning Viewfinder (Cyan circular frame with horizontal scanning laser)]", red_marker=True)
add_images([img("08_Face_Scan_Success.jpg")], width_inches=2.5)
add_p("[TODO: ADD SCREENSHOT - Face ID Recognition Failed / Retry Modal (Red circular frame with retry action)]", red_marker=True)

add_h4("Function Trigger")
add_p("From the Welcome Screen, tap to transition to Role Selection Screen. The customer taps \"QUÉT FACE ID ĐĂNG NHẬP\" (Scan Face ID to Login) to initiate facial biometric login, or taps \"BẮT ĐẦU NGAY\" (Start Now) under Guest card to proceed as a guest.", bold_prefix="Trigger Role Selection & Face Login:")
...
add_p("[TODO: ADD SCREENSHOT - Health Tag Personalized Recommendations Screen (Dietary tags: Eat Clean, Keto, Vegan)]", red_marker=True)
...
add_p("[TODO: ADD SCREENSHOT - Voice Recognition Query Results Screen (Parsed query and matched product list)]", red_marker=True)
...
add_h4("Business Rules")
add_p("Customer can add items to cart without triggering robot movement; supports quantity adjustments directly on screen.", bold_prefix="Cart Rules:")
...
add_p("[TODO: ADD SCREENSHOT - Robot Moving State Screen (NAVIGATING status with animated route progress)]", red_marker=True)
add_p("[TODO: ADD SCREENSHOT - Shelf Arrival & 30s Countdown Modal (ARRIVED status with shelf item pickup checklist)]", red_marker=True)
add_p("[TODO: ADD SCREENSHOT - 2D Supermarket Floor Map Viewer (ROS Pose & Lidar trajectory overlay)]", red_marker=True)

add_h4("Function Trigger")
add_p("On Cart Screen -> Customer clicks green button \"Robot dẫn theo giỏ hàng\" (Robot guides cart route), or on Product Detail Screen -> Customer clicks \"Dẫn đường\".", bold_prefix="Trigger:")
...
add_p(".NET Route Optimization Engine (Dijkstra TSP), SignalR Hub (RobotHub), ROS2 Navigation Stack, Lidar sensors, and Mecanum wheel odometry.", bold_prefix="System:")
add_p("Optimize shelf visiting sequence to minimize travel distance; render real-time stage progress, guide item retrieval, and autonomously return robot to docking station.", bold_prefix="Purpose:")
add_p("Cart list showing items, unit prices, quantity stepper, red delete icon, total cost (128.000đ); Cart guide interface with robot status header (\"RB001 Sẵn sàng\"), guidance state card (DISPATCHED, NAVIGATING, ARRIVED), next destination card (\"Kệ 3 - Thực Phẩm Tươi Sống\") with shelf item list; multi-stop route timeline (Origin Waypoint 7 -> Shelf 3 -> Return Waypoint 7); red cancel button.", bold_prefix="Interface (Based on UI Mockups):")

add_h4("Normal Flow")
add_p("1. The customer verifies items in Cart and clicks \"Robot dẫn theo giỏ hàng\".")
add_p("2. Backend receives product list and maps each item to corresponding Shelf ID and navigation Node ID.")
add_p("3. Dijkstra algorithm computes optimal shortest path visiting all required shelves and dispatches navigation command to robot via SignalR.")
...
add_p("5. Robot travels forward (MOVING), updating progress Leg 1/1.")
...
add_p("8. Robot continues guiding to remaining shelves until all items are gathered, then autonomously navigates back to docking station (Waypoint 7).")
...
add_h4("Business Rules")
add_p("Always appends start waypoint (Waypoint 7) and home return waypoint (Waypoint 7) to beginning and end of route; prevents automatic idle session logout while robot is actively navigating.", bold_prefix="Navigation Invariant:")

# ==============================================================================
# 3.2.8 Contextual Overlays & Autonomous Advertising
# ==============================================================================
add_h4("3.2.8 Contextual Overlays & Autonomous Advertising")
add_p("[TODO: ADD SCREENSHOT - Zone Ad Full-screen Slide-up Modal (Promotional video/banner when entering grocery aisle)]", red_marker=True)
add_p("[TODO: ADD SCREENSHOT - Route Preview Modal (Miniaturized 2D path preview modal)]", red_marker=True)
add_p("[TODO: ADD SCREENSHOT - Embedded Assistant Ad Display Banner (Contextual sponsor banner inside voice reply)]", red_marker=True)

add_h4("Function Trigger")
add_p("Robot navigates into geographical radius of a specific merchandise zone (GeofencingContext receives zoneEntered event or navigationStatus). Or customer clicks route preview icon.", bold_prefix="Trigger:")

add_h4("Timing / Frequency")
add_p("Automatically triggered as robot traverses aisles covered by active promotional sponsorship contracts.", bold_prefix="Timing:")

add_h4("Function Description")
add_p("Shopper walking near robot in aisles.", bold_prefix="Actors / Roles:")
add_p("Geofencing Engine, Dynamic Ad Playlist Service, TTS Voice Broadcaster, SignalR Navigation Hub.", bold_prefix="System:")
add_p("Deliver location-based contextual promotions directly at point of interest, increasing marketing revenue and enabling one-tap cart addition for featured specials.", bold_prefix="Purpose:")
add_p("Slide-up modal sheet with 16:9 hero banner, campaign headline, countdown progress bar, promotional price, action button \"Thêm vào giỏ ngay\", and close button [X].", bold_prefix="Interface (Based on UI Mockups):")

add_h4("Normal Flow")
add_p("1. Robot patrols or guides customer into Shelf 3 zone (Fresh Produce aisle).")
add_p("2. Geofencing event fires; system checks prioritized playlist allocated for this zone.")
add_p("3. ZoneAdOverlay slides up over current screen; speaker plays product endorsement: \"[Tên sản phẩm] - Giá ưu đãi chỉ [Giá] đồng. Quý khách có thể chạm vào màn hình để thêm vào giỏ hàng nhé!\".")
add_p("4. Countdown progress bar animates from 12 seconds to 0.")
add_p("5. Customer can click \"Thêm vào giỏ ngay\" to instantly purchase, or click close button to dismiss.")
add_p("6. Upon countdown completion, overlay smoothly slides down, restoring primary screen.")
...
add_h4("Data Processing")
add_p("Logs ad impressions and click events to /api/v1/ads/impressions with sessionId and robotCode for sponsor budget reconciliation.", bold_prefix="Ad Analytics Pipeline:")

add_h4("Business Rules")
add_p("Enforces 1.5-second click debounce to prevent spam clicks; yields screen priority unconditionally to emergency stop alerts or critical path corrections.", bold_prefix="Advertising Policy:")

print("All 8 sub-modules written in English with Vietnamese UI elements successfully!")
